...
const MemeAlza = () => {
  ...
  useEffect(() => {
    const cargarMemes = async () => {
      try {
        ...
        // Filtrar memes al alza
        const memesAlAlza = data
          .filter((meme) => meme.cambio24h > 0)
          .sort((a, b) => b.cambio24h - a.cambio24h); // Orden descendente
        setMemes(memesAlAlza);
      } catch (err) {
  ...
  const handleCompra = async (meme, e) => {
    e.stopPropagation(); // No navegar al hacer click en botón
  ...
  if (loading) return <p className="text-gray-400">Cargando memes al alza...</p>;
